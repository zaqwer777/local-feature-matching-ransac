...
def compute_feature_distances(features1, features2):
    ...
    for ind in range(len(features1)):
        for index in range(len(features2)):
            dists[ind][index] = np.linalg.norm(features1[ind] - features2[index])
...
def match_features(features1, features2, x1, y1, x2, y2):
    ...
    """
    confidences = np.zeros((x1.shape[0], 2))
    matches = np.zeros((x1.shape[0], 2))
    ###########################################################################
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    distance = compute_feature_distances(features1, features2)
    sort_desc = np.argsort(distance)[:, ::-1]
    count = 0
    for row in sort_desc:
        matches[count] = [sort_desc[row][0][0], sort_desc[row][0][x2.shape[0] - 1]]
        confidences[count] = distance[count][sort_desc[row][0][0]] / distance[count][sort_desc[row][0][x2.shape[0] - 1]]
        count += 1
